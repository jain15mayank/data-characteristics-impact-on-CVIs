...
def dim_reduction_PCA(data, cumulative_explained_variance_ratio_threshold=0.96, random_state=None):
    """
    ...
    for i in range(pca1.n_components_):
        ...
        if cumulative_EVR>=cumulative_explained_variance_ratio_threshold:
            break
    pca2 = PCA(n_components=i+1, random_state=random_state)
    reducedDimData = pca2.fit_transform(data)
    return reducedDimData, i+1

def elbowHeuristic_PCA(data, markX=None, markY=None, annotX=None, annotY=None, figPath=None, debug=False, random_state=None):
    """
